...
- `uv run python scripts/capture_kalshi_ws.py OUT [--overwrite] [--seconds 90] [--min-deltas 15] [--top 5] [--require-side yes|no]` captures live Kalshi WS orderbook frames into a fixture file. The one dev tool that authenticates (Kalshi key from `.env`; read-only market data). `OUT` is required and an existing file is refused without `--overwrite`, because the captures under `tests/fixtures/kalshi/` are pinned by the parser tests; `--help`, a bare invocation and a refused `OUT` all exit before config, keys or the network are touched. Re-capturing a pinned file is a test change: run the suite before committing it.
