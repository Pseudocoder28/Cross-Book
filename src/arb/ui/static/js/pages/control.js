...
function build() {
  ...
  const watchCard = card("WATCH SET",
    ...
    ui.gapNote,
    el("div", "sys-title cs2", "BULK SET: N CONFIRMED, SPREAD ACROSS EVENTS"),
    row("N", field("ctl-pairstop", () => (ctl && ctl.pairs_top),
      { numeric: true, width: "7ch" })),
    ptBtns,
    note("THIS SETS THE TRACKED FLAG ON THE N PAIRS IT PICKS AND CLEARS IT ON EVERY "
      + "OTHER PAIR. IT IS A SETTER, NOT A FILTER: ANYTHING IT DOES NOT PICK STOPS "
      + "BEING WATCHED. N=0 WATCHES NOTHING."),
    note("SCORES TIE — A WHOLE RUN CAN SIT AT 1.000 — SO THE PICK GOES ROUND-ROBIN "
      + "ACROSS KALSHI EVENTS: ONE FROM EACH, THEN A SECOND FROM EACH, UNTIL N IS "
      + "FULL. TEN STRIKE BANDS OF ONE LADDER ARE TEN ROWS BUT ONE BET. PAIRS WHOSE "
      + "MARKET HAS ALREADY CLOSED ARE SKIPPED, AND COUNTED IN THE ARMED SENTENCE."));
